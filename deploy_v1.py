...
def deploy_oic_integration(oic_url, bearer_token, iar_file_path, instance_name=None, enable_async_activation_mode=False):
    ...
    try:
        ...
        if response.status_code == 204:
            print(f"  Import for '{os.path.basename(iar_file_path)}' returned 204 No Content.")
            print("  This typically means the import was processed, but no integration ID was returned.")
            print("  Activation cannot proceed without an integration ID from the import response.")
            return False
        ...
        if import_result.get("status") == "SUCCESS":
            ...
            # Clarification: The integration_id is typically in CODE|VERSION format.
